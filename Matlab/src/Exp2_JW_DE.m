...
disp('The optimal number is 30');
disp('This will take about 1/2 hour to run!');
  fprintf('\n')
question='How many trials to run : ';
  z= input(question);

  % sequence Increment Totals -2 times number of trials as stim and no stim pause times
IT=1:2*z;

avg_interval = 30; % was 90
...
% The cue lites are on for 8 sec. then off 8 sec. before the onset of stim delivery.
...
% generate 2 random trials (RT) where light on but no stim
RTs=randperm(40,2);
RTa=RTs(1,1);
RTb=RTs(1,2);
NScount=1; % no Stim Counter

for pl = 1:z  % 40 is 3600/90, so average number of stims expected in an hour

  if pl == RTa || pl == RTb % based  on 5% of 30 trials that would be 2
    % lights on but not stim
pause(intervals(pl)); % Wait interval !!
 ...
 i=i+1;k=k+1;
   ...
   writeDigitalPin (m,'D7',0);
        SE(i)=toc(ST); % toc(ST); % no stim time value
        SC(k)=10;
        i=i+1;k=k+1;
   
  else % end if for the random trials & do stim
...
% Random trials
d=num2str(RTs);
D=['Random trial numbers with lite but no stim are: ',d];
RT=cellstr(D);

% Event code text
e=['Event time codes: 5/6/7/8 is start/end, 2 is lites on, 1 is stim on, 10 is no stim'];
E=cellstr(e);
...
FileName=['Exp2_',datestr(now, 'dd-mmm-yyyy'), '_',a];
 ...
 pathAndFilename = fullfile(myFile);xlswrite(FileName,RT, 'Sheet1','A4'); % Random Trials
 pathAndFilename = fullfile(myFile);xlswrite(FileName,E, 'Sheet1','A5'); % Event codes
 pathAndFilename = fullfile(myFile);xlswrite(FileName,GS, 'Sheet1','B6'); % event codes
 pathAndFilename = fullfile(myFile);xlswrite(FileName,FS, 'Sheet1','A6'); % Sequ times
