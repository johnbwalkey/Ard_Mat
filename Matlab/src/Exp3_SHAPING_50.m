% EXPERIMENT 3.Shaping at 50% srim in Boxes B & C.
...
% THIS IS SHAPING CODE - light comes on in B or C and if poked get stim
% then light in box 1 and can go to box 1 and poke for more stim !!!!!
...
A=['Experiment 3 (SHAPING) - Subject # ',a];
...
CT=cellstr(e);
g=[' stim on in B/C - 1/2 WITH 21 & 31 are 50% stim in B/C'];
SOn=cellstr(g);
f=[' time out B/C- 25/35, time out in stim box B/C - 45/55'];
CT1=cellstr(f);
h=[' lite on in B/C - 10/15, lite on in C- 50'];
LOn=cellstr(h);
...
FileName=['Exp3_',datestr(now, 'dd-mmm-yyyy'), '_',a];
 ...
 pathAndFilename = fullfile(myFile);xlswrite(FileName,SOn, 'Sheet1','A6'); % Codes text
 pathAndFilename = fullfile(myFile);xlswrite(FileName,CT1, 'Sheet1','A7'); % Codes text
 pathAndFilename = fullfile(myFile);xlswrite(FileName,LOn, 'Sheet1','A8'); % Codes text
